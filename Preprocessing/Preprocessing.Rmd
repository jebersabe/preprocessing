...
df_train1 %>% glimpse()
```
imputation <- mice(train, m=2, maxit = 2, method = 'cart', seed = 500)
train <- complete(imputation,2)
...
miceMod <- mice::mice(df_train2, m=2, maxit = 2, method = 'cart', seed = 500)
df_train2 <- mice::complete(miceMod, 1)
df_train2 %>% glimpse()
```
